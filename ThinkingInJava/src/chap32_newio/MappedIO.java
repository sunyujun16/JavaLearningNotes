...
public class MappedIO {
    ...
    private static final String FILENAME_T =
            "ThinkingInJava/src/chap32_newio/temp.tmp";
    ...
    private static Tester[] tests = {
            ...
            new Tester("Mapped Write") {
                ...
                public void test() {
                    try (
                            FileChannel fc =
                                    new RandomAccessFile(FILENAME_T, "rw")
                                            .getChannel()
                    ) {
                        IntBuffer ib =
                            ...
                            DataInputStream dis =
                                    new DataInputStream(
                                            new BufferedInputStream(
                                                    new FileInputStream(FILENAME_T)))
                    ) {
                        for (int i = 0; i < numOfInts; i++)
                            dis.readInt();
                    } catch (IOException e) {
                ...
                public void test() {
                    try (
                            FileChannel fc = new FileInputStream(
                                    new File(FILENAME_T)).getChannel()
                    ) {
                        IntBuffer ib =
                ...
                public void test() {
                    try (
                            FileChannel fc = new RandomAccessFile(
                                    new File(FILENAME_T), "rw").getChannel()
                    ) {
                        IntBuffer ib =
                                fc.map(FileChannel.MapMode.READ_WRITE,
                                        0, fc.size()).asIntBuffer();
